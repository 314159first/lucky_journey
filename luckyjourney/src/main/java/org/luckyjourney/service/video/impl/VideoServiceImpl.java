...
import org.luckyjourney.service.mq.UserDataPersistenceService;
import org.luckyjourney.service.mq.VideoBehaviorProducer;
...
public class VideoServiceImpl extends ServiceImpl<VideoMapper, Video> implements VideoService {
    ...
    @Autowired
    private VideoBehaviorProducer videoBehaviorProducer;
    ...
    public void adminDeleteVideo(Long id) {
        ...
        final Video video = this.getById(id);
        if (video == null) {
            throw new BaseException("删除指定的视频不存在");
        }
        final boolean b = removeById(id);
    ...
    public IPage<Video> searchVideo(String search, BasePage basePage, Long userId) {
        IPage<Video> esPage = searchService.searchVideos(search, basePage);
        if (esPage != null && esPage.getTotal() > 0) {
            setUserVoAndUrl(esPage.getRecords());
            addSearchHistory(userId, search);
            return esPage;
        }
        ...
        wrapper.eq(Video::getOpen, false);
        if (StringUtils.hasText(search) && search.contains("YV")) {
            wrapper.eq(Video::getYv, search);
        } else {
            wrapper.like(!ObjectUtils.isEmpty(search), Video::getTitle, search);
        }
        wrapper.orderByDesc(Video::getGmtCreated);
    ...
    private void addSearchHistory(Long userId, String search) {
    ...
    public boolean startVideo(Long videoId) {
        ...
        Long delta = result ? 1L : -1L;
        VideoBehaviorEvent event = VideoBehaviorEvent.build(VideoBehaviorEvent.TYPE_STAR, userId, videoId, null, delta);
        if (!videoBehaviorProducer.send(event)) {
            updateStar(video, delta);
        }
        // 获取标签
    ...
    public boolean favoritesVideo(Long fId, Long vId) {
        ...
        Long delta = favorites ? 1L : -1L;
        VideoBehaviorEvent event = VideoBehaviorEvent.build(VideoBehaviorEvent.TYPE_FAVORITE, UserHolder.get(), vId, fId, delta);
        if (!videoBehaviorProducer.send(event)) {
            updateFavorites(video, delta);
        }
    ...
    public boolean shareVideo(VideoShare videoShare) {
        ...
        if (result) {
            VideoBehaviorEvent event = VideoBehaviorEvent.build(VideoBehaviorEvent.TYPE_SHARE,
                    videoShare.getUserId(),
                    videoShare.getVideoId(),
                    null,
                    1L);
            if (!videoBehaviorProducer.send(event)) {
                updateShare(video, 1L);
            }
        }
        return result;
    }
    ...
    public void historyVideo(Long videoId, Long userId) {
        ...
        if (o == null) {
            ...
            VideoBehaviorEvent event = VideoBehaviorEvent.build(VideoBehaviorEvent.TYPE_HISTORY, userId, videoId, null, 1L);
            if (!videoBehaviorProducer.send(event)) {
                updateHistory(video, 1L);
                userDataPersistenceService.saveVideoHistory(userId, videoId);
            }
        }
    }
    ...
    public LinkedHashMap<String, List<Video>> getHistory(BasePage basePage) {
        ...
        if (ObjectUtils.isEmpty(typedTuples)) {
            return getHistoryFromDb(userId, basePage);
        }
    ...
    private LinkedHashMap<String, List<Video>> getHistoryFromDb(Long userId, BasePage basePage) {
        final List<UserVideoHistoryRecord> historyRecords = userDataPersistenceService.listVideoHistory(userId, basePage.getPage(), basePage.getLimit());
        if (ObjectUtils.isEmpty(historyRecords)) {
            return new LinkedHashMap<>();
        }
        final List<Long> videoIds = historyRecords.stream().map(UserVideoHistoryRecord::getVideoId).collect(Collectors.toList());
        final Map<Long, Video> videoMap = listByIds(videoIds).stream().collect(Collectors.toMap(Video::getId, Function.identity()));
        final List<Video> videos = new ArrayList<>(videoMap.values());
        setUserVoAndUrl(videos);

        final SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        final LinkedHashMap<String, List<Video>> result = new LinkedHashMap<>();
        for (UserVideoHistoryRecord record : historyRecords) {
            Video video = videoMap.get(record.getVideoId());
            if (video == null) {
                continue;
            }
            final String format = simpleDateFormat.format(record.getViewedAt());
            if (!result.containsKey(format)) {
                result.put(format, new ArrayList<>());
            }
            result.get(format).add(video);
